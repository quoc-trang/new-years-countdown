    ...
    static appendChild(ancestorNode, node, options) {
        if (node.nodeType === NodeTypeEnum.elementNode && node !== ancestorNode) {
            ...
            if (node.parentNode && node.parentNode.children) {
                ...
                if (index !== -1) {
                    for (const attributeName of NAMED_ITEM_ATTRIBUTES) {
                        ...
                        if (attribute) {
                            node.parentNode.children._removeNamedItem(node, attribute.value);
                        }
                    }
            ...
            for (const attributeName of NAMED_ITEM_ATTRIBUTES) {
                const attribute = node.attributes.getNamedItem(attributeName);
                if (attribute) {
                    ancestorNode.children._appendNamedItem(node, attribute.value);
                }
            }
        ...
        if (node.nodeType === NodeTypeEnum.elementNode) {
            ...
            if (index !== -1) {
                for (const attributeName of NAMED_ITEM_ATTRIBUTES) {
                    const attribute = node.attributes.getNamedItem(attributeName);
                    if (attribute) {
                        ancestorNode.children._removeNamedItem(node, attribute.value);
                    }
                }
    ...
    static insertBefore(ancestorNode, newNode, referenceNode, options) {
        ...
        if (newNode.nodeType === NodeTypeEnum.elementNode && referenceNode) {
            ...
            if (newNode.parentNode && newNode.parentNode.children) {
                ...
                if (index !== -1) {
                    for (const attributeName of NAMED_ITEM_ATTRIBUTES) {
                        ...
                        if (attribute) {
                            (newNode.parentNode.children)._removeNamedItem(newNode, attribute.value);
                        }
                    }
            ...
            for (const attributeName of NAMED_ITEM_ATTRIBUTES) {
                const attribute = newNode.attributes.getNamedItem(attributeName);
                if (attribute) {
                    ancestorNode.children._appendNamedItem(newNode, attribute.value);
                }
            }
